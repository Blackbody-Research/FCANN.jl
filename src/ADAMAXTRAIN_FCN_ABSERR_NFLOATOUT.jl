...
include("FCN_ABSERR_NFLOATOUT_COSTFUNCTIONS.jl")

include("FCN_ABSERR_NFLOATOUT_AUXFUNCTIONS.jl")
...
function checkNumGrad(lambda; hidden_layers=[5, 5])
	srand(1234)
	m = 100
	input_layer_size = 3
	output_layer_size = 2
...
function ADAMAXTrainNN(input_data, output_data, batchSize, T0, B0, N, input_layer_size, hidden_layers, lambda, c; alpha=0.002f0, R = 0.1f0, printProgress = false, dropout = 0.0f0)
