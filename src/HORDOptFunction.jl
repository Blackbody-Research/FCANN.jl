...
function f(X, Xtest, Y, Ytest, rawParams, pconv, batchSize, OPT_PARAMS)
    ...
    N = params[3] #number of training epochs
    numParams =params[4] #target number of training parameters
    L = params[5] #number of layers
    c = params[6] #max norm reg constant
    ensnum = params[7] #number of ensemble networks

    parameterNames = ["Alpha", "R", "N", "Target_Params", "Layers", "Max_Norm", "Num_Networks"]
    ...
    #do multitrain
        ...
        bootstrapOut = pmap(1:ensnum) do foo
            ...
            (T, B, bestCost, costRecord, timeRecord, GFLOPS) = if alpha == 0.0f0
                srand(1234 + foo - 1)
                ADAMAXTrainNN(X, Y, batchSize, T0, B0, 1, M, hidden_layers, 0.0f0, c; alpha=alpha, R = R, printProgress = printProg, dropout = 0.0f0)
            else
                srand(1234 + foo - 1)
                ADAMAXTrainNN(X, Y, batchSize, T0, B0, N, M, hidden_layers, 0.0f0, c; alpha=alpha, R = R, printProgress = printProg, dropout = 0.0f0)
            end
...
function HORDOpt(name, OPT_PARAMS, trialID, Nmax, ISP = [])
    parameterNames = ["Alpha", "R", "N", "Target_Params", "Layers", "Max_Norm", "Num_Networks"]

    #vector of functions to ensure parameters are of the correct type
    pconvert = [Float32, Float32, a -> round(Int64, a), a -> round(Int64, a), a -> round(Int64, a), a -> Float32(1/a), a -> round(Int64, a)]
    ...
    header = [parameterNames[h]' "Actual Num Params" "Hidden Layer Size" "Training Error" "Test Error"]
    body = [mapreduce(a -> a[h]', vcat, params) reduce(vcat, outputs) Fs]

    writecsv(string(name, "_", fixedName, "_HORD_results_trial_", trialID, ".csv"), [header; body])

    HORDHistory = [mapreduce(x -> x', vcat, Xs) Fs]
    writecsv(string(name, "_", fixedName, "_HORDHistory_trial_", trialID, ".csv"), HORDHistory)
