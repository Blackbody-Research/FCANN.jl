...
train_trials([(Xtrain, ytrain)], 512, [2, 2]; N = 10)
train_trials([(Xtrain, ytrain)], 512, [2, 2]; N = 10, trial_lists = Dict([:seed => 1:5, :use_μP => [true, false], :resLayers => [0, 1]]), costFunc = "normLogErr")
train_trials([(Xtrain, ytrain), (Xtest, ytest)], 512, [2, 2]; N = 10)